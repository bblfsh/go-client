...
var (
	...
	ErrLangNotSupported = errors.NewKind("language %q is not supported in current configuration")
	// ErrNotImplemented is the error that must be returned if some methods are not implemented
...
func NewClientContext(ctx context.Context, endpoint string, options ...grpc.DialOption) (*Client, error) {
	...
	case strings.Contains(endpoint, ","):
		...
		return NewClientWithConnectionsContext(func(ctx context.Context, lang string) (*grpc.ClientConn, error) {
			e, ok := endpoints[lang]
			if !ok {
				return nil, ErrLangNotSupported.New(lang)
			}
